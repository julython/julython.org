...
JULY.CommitCollection = Backbone.Collection.extend({
    ...
    var self = this;
    this._pushStream.onmessage = function(message) {
      console.log("-- New commit from PushStream: " + message);
      self.unshift(message);
    };
    ...
    var channel = (this.projectId) ? "project-" + this.projectId : (this.userId) ? "user-" + this.userId: "all";
